...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Customer.io for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      return { source: "customer-io", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a Customer.io page and return cleaned text content.",
