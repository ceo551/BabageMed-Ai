...
const SEARCH_URL  = "https://www.ema.europa.eu/en/search?search_api_fulltext={q}";
const SEL_RESULT  = ".views-row";
const SEL_TITLE   = "h3 a, h2 a";
const SEL_LINK    = "h3 a, h2 a";
const SEL_SNIPPET = ".search-snippet-info";
const ORIGIN      = "https://www.ema.europa.eu";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search European Medicines Agency for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://www.ema.europa.eu").toString(); } catch { return; }
      ...
      return { source: "ema", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a European Medicines Agency page and return cleaned text content.",
    ...
    handler: async ({ url }) => {
      ...
      try {
        const u = new URL(url);
        const expected = new URL("https://www.ema.europa.eu").hostname;
