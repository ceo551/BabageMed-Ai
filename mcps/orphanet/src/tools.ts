...
const SEARCH_URL  = "https://www.orpha.net/en/disease/search?query={q}";
const SEL_RESULT  = ".result-item, .listing-item";
const SEL_TITLE   = "a.title, h3 a";
const SEL_LINK    = "a.title, h3 a";
const SEL_SNIPPET = ".description";
const ORIGIN      = "https://www.orpha.net";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Orphanet for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://www.orpha.net").toString(); } catch { return; }
      ...
      return { source: "orphanet", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a Orphanet page and return cleaned text content.",
    ...
    handler: async ({ url }) => {
      ...
      try {
        const u = new URL(url);
        const expected = new URL("https://www.orpha.net").hostname;
