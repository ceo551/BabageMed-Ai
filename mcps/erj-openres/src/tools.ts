...
const SEARCH_URL  = "https://openres.ersjournals.com/search/{q}";
...
const ORIGIN      = "https://openres.ersjournals.com";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search ERJ Open Research for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://openres.ersjournals.com").toString(); } catch { return; }
      ...
      return { source: "erj-openres", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a ERJ Open Research page and return cleaned text content.",
