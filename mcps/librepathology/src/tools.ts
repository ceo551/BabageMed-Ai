...
const SEARCH_URL  = "https://librepathology.org/w/index.php?search={q}";
const SEL_RESULT  = ".mw-search-result, li.mw-search-result-ns-0";
const SEL_TITLE   = ".mw-search-result-heading a";
const SEL_LINK    = ".mw-search-result-heading a";
const SEL_SNIPPET = ".searchresult";
const ORIGIN      = "https://librepathology.org";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Libre Pathology for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://librepathology.org").toString(); } catch { return; }
      ...
      return { source: "librepathology", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a Libre Pathology page and return cleaned text content.",
    ...
    handler: async ({ url }) => {
      ...
      try {
        const u = new URL(url);
        const expected = new URL("https://librepathology.org").hostname;
