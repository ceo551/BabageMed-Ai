...
// Microsoft SQL Server — connector observed in the Perplexity Computer Connectors
...
// authenticate.
const HOMEPAGE = "https://www.microsoft.com/sql-server";
const NOT_CONFIGURED = {
  status: "not-configured",
  message: "Microsoft SQL Server connector is registered but not yet authenticated. Add credentials in Settings → Connectors.",
  homepage: HOMEPAGE,
};
...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Microsoft SQL Server. Returns 'not-configured' until OAuth/API credentials are supplied.",
  ...
  server.tool({
    name: "fetch",
    description: "Fetch a record from Microsoft SQL Server by id or URL. Stub until configured.",
