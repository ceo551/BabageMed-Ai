...
const SEARCH_URL  = "https://www.jnma.com.np/jnma/index.php/jnma/search/search?query={q}";
const SEL_RESULT  = ".article-summary";
const SEL_TITLE   = "a.title";
const SEL_LINK    = "a.title";
const SEL_SNIPPET = ".abstract";
const ORIGIN      = "https://www.jnma.com.np";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Journal of Nepal Medical Association for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://www.jnma.com.np").toString(); } catch { return; }
      ...
      return { source: "jnma", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a Journal of Nepal Medical Association page and return cleaned text content.",
