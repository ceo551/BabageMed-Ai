...
// Cloudinary — connector observed in the Perplexity Computer Connectors
...
const NOT_CONFIGURED = {
  ...
  message: "Cloudinary connector is registered but not yet authenticated. Add credentials in Settings → Connectors.",
  homepage: HOMEPAGE,
};
...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Cloudinary. Returns 'not-configured' until OAuth/API credentials are supplied.",
  ...
  server.tool({
    name: "fetch",
    description: "Fetch a record from Cloudinary by id or URL. Stub until configured.",
