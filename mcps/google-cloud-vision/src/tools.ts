...
// Google Cloud Vision — connector observed in the Perplexity Computer Connectors
...
// authenticate.
const HOMEPAGE = "https://cloud.google.com/vision";
const NOT_CONFIGURED = {
  status: "not-configured",
  message: "Google Cloud Vision connector is registered but not yet authenticated. Add credentials in Settings → Connectors.",
  homepage: HOMEPAGE,
};
...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Google Cloud Vision. Returns 'not-configured' until OAuth/API credentials are supplied.",
  ...
  server.tool({
    name: "fetch",
    description: "Fetch a record from Google Cloud Vision by id or URL. Stub until configured.",
