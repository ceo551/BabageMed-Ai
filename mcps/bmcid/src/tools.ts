...
const SEARCH_URL  = "https://bmcinfectdis.biomedcentral.com/search?query={q}";
...
const ORIGIN      = "https://bmcinfectdis.biomedcentral.com";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search BMC Infectious Diseases for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://bmcinfectdis.biomedcentral.com").toString(); } catch { return; }
      ...
      return { source: "bmcid", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a BMC Infectious Diseases page and return cleaned text content.",
