...
const SEARCH_URL  = "https://www.jacc.org/action/doSearch?AllField={q}";
...
const ORIGIN      = "https://www.jacc.org";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search JACC Journals for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://www.jacc.org").toString(); } catch { return; }
      ...
      return { source: "jacc", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a JACC Journals page and return cleaned text content.",
