...
// Google Search Console — connector observed in the Perplexity Computer Connectors
...
// authenticate.
const HOMEPAGE = "https://search.google.com/search-console";
const NOT_CONFIGURED = {
  status: "not-configured",
  message: "Google Search Console connector is registered but not yet authenticated. Add credentials in Settings → Connectors.",
  homepage: HOMEPAGE,
};
...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Google Search Console. Returns 'not-configured' until OAuth/API credentials are supplied.",
  ...
  server.tool({
    name: "fetch",
    description: "Fetch a record from Google Search Console by id or URL. Stub until configured.",
