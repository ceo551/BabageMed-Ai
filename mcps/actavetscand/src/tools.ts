...
const SEARCH_URL  = "https://actavetscand.biomedcentral.com/search?query={q}";
...
const ORIGIN      = "https://actavetscand.biomedcentral.com";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Acta Veterinaria Scandinavica for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://actavetscand.biomedcentral.com").toString(); } catch { return; }
      ...
      return { source: "actavetscand", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a Acta Veterinaria Scandinavica page and return cleaned text content.",
