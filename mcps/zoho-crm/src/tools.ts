...
// Zoho CRM — connector observed in the Perplexity Computer Connectors
...
// authenticate.
const HOMEPAGE = "https://www.zoho.com/crm";
const NOT_CONFIGURED = {
  status: "not-configured",
  message: "Zoho CRM connector is registered but not yet authenticated. Add credentials in Settings → Connectors.",
  homepage: HOMEPAGE,
};
...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Zoho CRM. Returns 'not-configured' until OAuth/API credentials are supplied.",
  ...
  server.tool({
    name: "fetch",
    description: "Fetch a record from Zoho CRM by id or URL. Stub until configured.",
