...
const SEARCH_URL  = "https://www.dermnetnz.org/search/?q={q}";
const SEL_RESULT  = ".search-result, article";
const SEL_TITLE   = "h3 a, h2 a";
const SEL_LINK    = "h3 a, h2 a";
const SEL_SNIPPET = "p";
const ORIGIN      = "https://www.dermnetnz.org";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search DermNet NZ for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://www.dermnetnz.org").toString(); } catch { return; }
      ...
      return { source: "dermnet", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a DermNet NZ page and return cleaned text content.",
    ...
    handler: async ({ url }) => {
      ...
      try {
        const u = new URL(url);
        const expected = new URL("https://www.dermnetnz.org").hostname;
