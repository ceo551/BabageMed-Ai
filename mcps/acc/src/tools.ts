...
const SEARCH_URL  = "https://www.acc.org/search?q={q}";
const SEL_RESULT  = ".search-result, article";
const SEL_TITLE   = "h3 a, h2 a";
const SEL_LINK    = "h3 a, h2 a";
const SEL_SNIPPET = ".excerpt";
const ORIGIN      = "https://www.acc.org";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search American College of Cardiology for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://www.acc.org").toString(); } catch { return; }
      ...
      return { source: "acc", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a American College of Cardiology page and return cleaned text content.",
