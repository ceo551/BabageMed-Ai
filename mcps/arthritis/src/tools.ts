...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search arthritis.",
    input: z.object({ query: z.string().min(1), limit: z.number().int().min(1).max(50).optional() }),
    handler: async ({ query, limit = 10 }) => {
      const url = "https://www.arthritis.org/search?q={q}".replace("{q}", encodeURIComponent(query));
      ...
      $("a").each((_, a) => {
        ...
        try {
          const abs = new URL(href, "https://www.arthritis.org").toString();
          if (!abs.startsWith("https://www.arthritis.org")) return;
          if (results.some(r => r.url === abs)) return;
          results.push({ title: text.slice(0, 200), url: abs, snippet: "" });
        } catch {}
      });
      return { source: "arthritis", query, count: results.length, results };
    },
  });
  server.tool({
    name: "fetch",
    description: "Fetch a arthritis URL and return cleaned text content.",
