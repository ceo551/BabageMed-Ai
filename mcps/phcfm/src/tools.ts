...
const SEARCH_URL  = "https://phcfm.org/index.php/phcfm/search/search?query={q}";
const SEL_RESULT  = ".article-summary, li";
const SEL_TITLE   = "a.title, h3 a";
const SEL_LINK    = "a.title, h3 a";
const SEL_SNIPPET = ".abstract";
const ORIGIN      = "https://phcfm.org";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search African Journal of Primary Health Care & Family Medicine for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://phcfm.org").toString(); } catch { return; }
      ...
      return { source: "phcfm", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a African Journal of Primary Health Care & Family Medicine page and return cleaned text content.",
    ...
    handler: async ({ url }) => {
      ...
      try {
        const u = new URL(url);
        const expected = new URL("https://phcfm.org").hostname;
