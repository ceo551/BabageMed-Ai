...
const SEARCH_URL  = "https://bmcpulmmed.biomedcentral.com/search?query={q}";
...
const ORIGIN      = "https://bmcpulmmed.biomedcentral.com";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search BMC Pulmonary Medicine for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://bmcpulmmed.biomedcentral.com").toString(); } catch { return; }
      ...
      return { source: "bmcpulm", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a BMC Pulmonary Medicine page and return cleaned text content.",
