...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Findymail for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      return { source: "findymail", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a Findymail page and return cleaned text content.",
