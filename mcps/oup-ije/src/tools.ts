...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search International Journal of Epidemiology for a query and return links + snippets.",
    input: z.object({ query: z.string().min(1), limit: z.number().int().min(1).max(50).optional() }),
    handler: async ({ query, limit = 10 }) => {
      const url = "https://academic.oup.com/ije" + "/?s=" + encodeURIComponent(query);
      ...
      $("a").each((_, a) => {
        ...
        try {
          const abs = new URL(href, "https://academic.oup.com/ije").toString();
      ...
      return { source: "oup-ije", query, count: results.length, results };
    },
  });
