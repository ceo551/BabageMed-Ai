...
const SEARCH_URL  = "https://academic.oup.com/ije/search-results?page=1&q={q}";
...
const ORIGIN      = "https://academic.oup.com/ije";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search International Journal of Epidemiology for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://academic.oup.com/ije").toString(); } catch { return; }
      ...
      return { source: "oup-ije", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a International Journal of Epidemiology page and return cleaned text content.",
    ...
    handler: async ({ url }) => {
      ...
      try {
        const u = new URL(url);
        const expected = new URL("https://academic.oup.com/ije").hostname;
