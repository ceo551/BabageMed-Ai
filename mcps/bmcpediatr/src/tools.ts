...
const SEARCH_URL  = "https://bmcpediatr.biomedcentral.com/search?query={q}";
...
const ORIGIN      = "https://bmcpediatr.biomedcentral.com";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search BMC Pediatrics for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://bmcpediatr.biomedcentral.com").toString(); } catch { return; }
      ...
      return { source: "bmcpediatr", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a BMC Pediatrics page and return cleaned text content.",
