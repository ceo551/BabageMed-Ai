...
const SEARCH_URL  = "https://insightsimaging.springeropen.com/search?query={q}";
...
const ORIGIN      = "https://insightsimaging.springeropen.com";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Insights into Imaging for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://insightsimaging.springeropen.com").toString(); } catch { return; }
      ...
      return { source: "insightsimaging", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a Insights into Imaging page and return cleaned text content.",
