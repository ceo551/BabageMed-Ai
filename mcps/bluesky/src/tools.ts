...
// Bluesky — connector observed in the Perplexity Computer Connectors
...
// authenticate.
const HOMEPAGE = "https://bsky.app";
const NOT_CONFIGURED = {
  status: "not-configured",
  message: "Bluesky connector is registered but not yet authenticated. Add credentials in Settings → Connectors.",
  homepage: HOMEPAGE,
};
...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Bluesky. Returns 'not-configured' until OAuth/API credentials are supplied.",
  ...
  server.tool({
    name: "fetch",
    description: "Fetch a record from Bluesky by id or URL. Stub until configured.",
