...
// Canva Enterprise — connector observed in the Perplexity Computer Connectors
...
// authenticate.
const HOMEPAGE = "https://www.canva.com/enterprise";
const NOT_CONFIGURED = {
  status: "not-configured",
  message: "Canva Enterprise connector is registered but not yet authenticated. Add credentials in Settings → Connectors.",
  homepage: HOMEPAGE,
};
...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Canva Enterprise. Returns 'not-configured' until OAuth/API credentials are supplied.",
    input: z.object({ query: z.string().min(1), limit: z.number().int().min(1).max(50).optional() }),
    handler: async ({ query, limit = 10 }) => ({
      source: "Canva Enterprise",
      query,
      limit,
      count: 0,
      results: [],
  ...
  server.tool({
    name: "fetch",
    description: "Fetch a record from Canva Enterprise by id or URL. Stub until configured.",
