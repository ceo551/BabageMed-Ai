...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search rxlist.",
    input: z.object({ query: z.string().min(1), limit: z.number().int().min(1).max(50).optional() }),
    handler: async ({ query, limit = 10 }) => {
      const url = "https://www.rxlist.com/search/rxl/{q}".replace("{q}", encodeURIComponent(query));
      ...
      $("a").each((_, a) => {
        ...
        try {
          const abs = new URL(href, "https://www.rxlist.com").toString();
          if (!abs.startsWith("https://www.rxlist.com")) return;
          if (results.some(r => r.url === abs)) return;
          results.push({ title: text.slice(0, 200), url: abs, snippet: "" });
        } catch {}
      });
      return { source: "rxlist", query, count: results.length, results };
    },
  });
  server.tool({
    name: "fetch",
    description: "Fetch a rxlist URL and return cleaned text content.",
    ...
    handler: async ({ url }) => {
      ...
      const sel = ".article-content, main";
