...
const SEARCH_URL  = "https://bmcpublichealth.biomedcentral.com/search?query={q}";
...
const ORIGIN      = "https://bmcpublichealth.biomedcentral.com";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search BMC Public Health for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://bmcpublichealth.biomedcentral.com").toString(); } catch { return; }
      ...
      return { source: "bmcph", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a BMC Public Health page and return cleaned text content.",
