...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search iowaprotocols.",
    input: z.object({ query: z.string().min(1), limit: z.number().int().min(1).max(50).optional() }),
    handler: async ({ query, limit = 10 }) => {
      const url = "https://webeye.ophth.uiowa.edu/eyeforum/cases/search.htm?q={q}".replace("{q}", encodeURIComponent(query));
      ...
      $("a").each((_, a) => {
        ...
        try {
          const abs = new URL(href, "https://webeye.ophth.uiowa.edu").toString();
          if (!abs.startsWith("https://webeye.ophth.uiowa.edu")) return;
          if (results.some(r => r.url === abs)) return;
          results.push({ title: text.slice(0, 200), url: abs, snippet: "" });
        } catch {}
      });
      return { source: "iowaprotocols", query, count: results.length, results };
    },
  });
  server.tool({
    name: "fetch",
    description: "Fetch a iowaprotocols URL and return cleaned text content.",
