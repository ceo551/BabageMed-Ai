...
const SEARCH_URL  = "https://link.springer.com/journal/330/search?query={q}";
const SEL_RESULT  = ".app-card, .c-list-group__item, .toc-item";
...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search European Radiology (Springer) for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://link.springer.com/journal/330").toString(); } catch { return; }
      ...
      return { source: "springer-eurrad", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a European Radiology (Springer) page and return cleaned text content.",
    ...
    handler: async ({ url }) => {
      ...
      try {
        const u = new URL(url);
        const expected = new URL("https://link.springer.com/journal/330").hostname;
