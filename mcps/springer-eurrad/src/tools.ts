...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search European Radiology (Springer) for a query and return links + snippets.",
    input: z.object({ query: z.string().min(1), limit: z.number().int().min(1).max(50).optional() }),
    handler: async ({ query, limit = 10 }) => {
      const url = "https://link.springer.com/journal/330" + "/?s=" + encodeURIComponent(query);
      ...
      $("a").each((_, a) => {
        ...
        try {
          const abs = new URL(href, "https://link.springer.com/journal/330").toString();
          if (!abs.startsWith("https://link.springer.com")) return;
          if (results.some(x => x.url === abs)) return;
          results.push({ title: text.slice(0, 200), url: abs, snippet: "" });
        } catch {}
      });
      return { source: "springer-eurrad", query, count: results.length, results };
    },
  });
