...
const SEARCH_URL  = "https://www.capc.org/search/?q={q}";
const SEL_RESULT  = "article, .post, .search-result, li";
const SEL_TITLE   = "h1, h2, h3, .title, a";
const SEL_LINK    = "a";
const SEL_SNIPPET = "p, .excerpt, .summary";
const ORIGIN      = "https://www.capc.org";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Center to Advance Palliative Care for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://www.capc.org").toString(); } catch { return; }
      ...
      return { source: "capc", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a Center to Advance Palliative Care page and return cleaned text content.",
