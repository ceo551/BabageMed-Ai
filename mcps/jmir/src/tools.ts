...
const SEARCH_URL  = "https://www.jmir.org/search?q={q}";
...
const ORIGIN      = "https://www.jmir.org";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search JMIR for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://www.jmir.org").toString(); } catch { return; }
      ...
      return { source: "jmir", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a JMIR page and return cleaned text content.",
