...
const SEARCH_URL  = "https://www.dovepress.com/search.t?text={q}";
const SEL_RESULT  = ".article-listing-item";
const SEL_TITLE   = ".article-title a";
const SEL_LINK    = ".article-title a";
const SEL_SNIPPET = ".article-abstract";
const ORIGIN      = "https://www.dovepress.com";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Dove Press for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://www.dovepress.com").toString(); } catch { return; }
      ...
      return { source: "dovepress", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a Dove Press page and return cleaned text content.",
    ...
    handler: async ({ url }) => {
      ...
      try {
        const u = new URL(url);
        const expected = new URL("https://www.dovepress.com").hostname;
