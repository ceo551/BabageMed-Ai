...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search clevelandclinic.",
    input: z.object({ query: z.string().min(1), limit: z.number().int().min(1).max(50).optional() }),
    handler: async ({ query, limit = 10 }) => {
      const url = "https://my.clevelandclinic.org/search?q={q}".replace("{q}", encodeURIComponent(query));
      const r = await scraper.fetchHtml(url, { browser: true });
      const $ = r.$;
      const results: any[] = [];
      $(".search__result").each((_, el) => {
        if (results.length >= limit) return;
        const a = $(el).find("a").first();
        const href = a.attr("href"); if (!href) return;
        try {
          const abs = new URL(href, "https://my.clevelandclinic.org").toString();
          if (!abs.startsWith("https://my.clevelandclinic.org")) return;
          results.push({ title: a.text().trim(), url: abs, snippet: $(el).find("p").first().text().trim() });
        } catch {}
      });
      return { source: "clevelandclinic", query, count: results.length, results };
    },
  });
  server.tool({
    name: "fetch",
    description: "Fetch a clevelandclinic URL and return cleaned text content.",
