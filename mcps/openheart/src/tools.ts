...
const SEARCH_URL  = "https://openheart.bmj.com/search/{q}";
...
const ORIGIN      = "https://openheart.bmj.com";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Open Heart (BMJ OA) for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://openheart.bmj.com").toString(); } catch { return; }
      ...
      return { source: "openheart", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a Open Heart (BMJ OA) page and return cleaned text content.",
