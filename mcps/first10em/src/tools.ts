...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search first10em.",
    input: z.object({ query: z.string().min(1), limit: z.number().int().min(1).max(50).optional() }),
    handler: async ({ query, limit = 10 }) => {
      const url = "https://first10em.com/?s={q}".replace("{q}", encodeURIComponent(query));
      ...
      $("a").each((_, a) => {
        ...
        try {
          const abs = new URL(href, "https://first10em.com").toString();
          if (!abs.startsWith("https://first10em.com")) return;
          if (results.some(r => r.url === abs)) return;
          results.push({ title: text.slice(0, 200), url: abs, snippet: "" });
        } catch {}
      });
      return { source: "first10em", query, count: results.length, results };
    },
  });
  server.tool({
    name: "fetch",
    description: "Fetch a first10em URL and return cleaned text content.",
