...
const SEARCH_URL  = "https://health-products.canada.ca/dpd-bdpp//?s={q}";
...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Health Canada drug database for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://health-products.canada.ca/dpd-bdpp/").toString(); } catch { return; }
      ...
      return { source: "healthcanada-drugs", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a Health Canada drug database page and return cleaned text content.",
    ...
    handler: async ({ url }) => {
      ...
      try {
        const u = new URL(url);
        const expected = new URL("https://health-products.canada.ca/dpd-bdpp/").hostname;
