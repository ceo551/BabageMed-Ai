...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Health Canada drug database for a query and return links + snippets.",
    input: z.object({ query: z.string().min(1), limit: z.number().int().min(1).max(50).optional() }),
    handler: async ({ query, limit = 10 }) => {
      const url = "https://health-products.canada.ca/dpd-bdpp/" + "/?s=" + encodeURIComponent(query);
      ...
      $("a").each((_, a) => {
        ...
        try {
          const abs = new URL(href, "https://health-products.canada.ca/dpd-bdpp/").toString();
          if (!abs.startsWith("https://health-products.canada.ca")) return;
          if (results.some(x => x.url === abs)) return;
          results.push({ title: text.slice(0, 200), url: abs, snippet: "" });
        } catch {}
      });
      return { source: "healthcanada-drugs", query, count: results.length, results };
    },
  });
