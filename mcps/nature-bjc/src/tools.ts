...
const SEARCH_URL  = "https://www.nature.com/bjc//search?q={q}";
const SEL_RESULT  = ".c-card, article";
const SEL_TITLE   = "h3 a, .c-card__title a";
const SEL_LINK    = "h3 a, .c-card__title a";
const SEL_SNIPPET = ".c-card__summary";
const ORIGIN      = "https://www.nature.com/bjc";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search British Journal of Cancer (Nature) for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://www.nature.com/bjc/").toString(); } catch { return; }
      ...
      return { source: "nature-bjc", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a British Journal of Cancer (Nature) page and return cleaned text content.",
    ...
    handler: async ({ url }) => {
      ...
      try {
        const u = new URL(url);
        const expected = new URL("https://www.nature.com/bjc/").hostname;
