...
const SEARCH_URL  = "https://www.escardio.org/search?q={q}";
const SEL_RESULT  = ".search-result, .listing-item";
const SEL_TITLE   = "h3 a";
const SEL_LINK    = "h3 a";
const SEL_SNIPPET = ".excerpt";
const ORIGIN      = "https://www.escardio.org";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search European Society of Cardiology for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://www.escardio.org").toString(); } catch { return; }
      ...
      return { source: "escardio", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a European Society of Cardiology page and return cleaned text content.",
    ...
    handler: async ({ url }) => {
      ...
      try {
        const u = new URL(url);
        const expected = new URL("https://www.escardio.org").hostname;
