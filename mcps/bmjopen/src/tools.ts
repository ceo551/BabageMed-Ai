...
const SEARCH_URL  = "https://bmjopen.bmj.com/search/{q}";
...
const ORIGIN      = "https://bmjopen.bmj.com";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search BMJ Open for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://bmjopen.bmj.com").toString(); } catch { return; }
      ...
      return { source: "bmjopen", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a BMJ Open page and return cleaned text content.",
