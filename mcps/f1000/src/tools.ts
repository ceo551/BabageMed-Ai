...
const SEARCH_URL  = "https://f1000research.com/search?q={q}";
const SEL_RESULT  = ".f1000-search-result, .article-summary";
const SEL_TITLE   = "h3 a";
const SEL_LINK    = "h3 a";
const SEL_SNIPPET = ".article-summary__abstract";
const ORIGIN      = "https://f1000research.com";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search F1000Research for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://f1000research.com").toString(); } catch { return; }
      ...
      return { source: "f1000", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a F1000Research page and return cleaned text content.",
    ...
    handler: async ({ url }) => {
      ...
      try {
        const u = new URL(url);
        const expected = new URL("https://f1000research.com").hostname;
