...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search renalfellow.",
    input: z.object({ query: z.string().min(1), limit: z.number().int().min(1).max(50).optional() }),
    handler: async ({ query, limit = 10 }) => {
      const url = "https://www.renalfellow.org/?s={q}".replace("{q}", encodeURIComponent(query));
      ...
      $("a").each((_, a) => {
        ...
        try {
          const abs = new URL(href, "https://www.renalfellow.org").toString();
          if (!abs.startsWith("https://www.renalfellow.org")) return;
          if (results.some(r => r.url === abs)) return;
          results.push({ title: text.slice(0, 200), url: abs, snippet: "" });
        } catch {}
      });
      return { source: "renalfellow", query, count: results.length, results };
    },
  });
  server.tool({
    name: "fetch",
    description: "Fetch a renalfellow URL and return cleaned text content.",
