...
// Google Merchant Center — connector observed in the Perplexity Computer Connectors
...
// authenticate.
const HOMEPAGE = "https://merchants.google.com";
const NOT_CONFIGURED = {
  status: "not-configured",
  message: "Google Merchant Center connector is registered but not yet authenticated. Add credentials in Settings → Connectors.",
  homepage: HOMEPAGE,
};
...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Google Merchant Center. Returns 'not-configured' until OAuth/API credentials are supplied.",
  ...
  server.tool({
    name: "fetch",
    description: "Fetch a record from Google Merchant Center by id or URL. Stub until configured.",
