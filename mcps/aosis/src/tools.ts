...
const SEARCH_URL  = "https://journals.aosis.co.za/index.php/index/search/search?query={q}";
const SEL_RESULT  = ".result-set li, .ojs-result-list li";
const SEL_TITLE   = "a.title, h3 a";
const SEL_LINK    = "a.title, h3 a";
const SEL_SNIPPET = ".meta";
const ORIGIN      = "https://journals.aosis.co.za";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search AOSIS (African journals) for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://journals.aosis.co.za").toString(); } catch { return; }
      ...
      return { source: "aosis", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a AOSIS (African journals) page and return cleaned text content.",
