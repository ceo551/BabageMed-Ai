...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search YouTube Analytics for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      return { source: "youtube-analytics", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a YouTube Analytics page and return cleaned text content.",
