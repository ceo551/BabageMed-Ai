...
const SEARCH_URL  = "https://ashpublications.org/searchresults?q={q}";
const SEL_RESULT  = ".al-article-items";
const SEL_TITLE   = ".sri-title a";
const SEL_LINK    = ".sri-title a";
const SEL_SNIPPET = ".sri-snippet";
const ORIGIN      = "https://ashpublications.org";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search ASH Publications for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://ashpublications.org").toString(); } catch { return; }
      ...
      return { source: "ashpub", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a ASH Publications page and return cleaned text content.",
    ...
    handler: async ({ url }) => {
      ...
      try {
        const u = new URL(url);
        const expected = new URL("https://ashpublications.org").hostname;
