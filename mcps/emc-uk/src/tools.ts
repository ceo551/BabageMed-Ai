...
const SEARCH_URL  = "https://www.medicines.org.uk/emc/search?q={q}";
const SEL_RESULT  = ".product-name, li.search-result";
const SEL_TITLE   = "a";
const SEL_LINK    = "a";
const SEL_SNIPPET = ".description";
const ORIGIN      = "https://www.medicines.org.uk/emc";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Electronic Medicines Compendium (UK) for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://www.medicines.org.uk/emc").toString(); } catch { return; }
      ...
      return { source: "emc-uk", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a Electronic Medicines Compendium (UK) page and return cleaned text content.",
    ...
    handler: async ({ url }) => {
      ...
      try {
        const u = new URL(url);
        const expected = new URL("https://www.medicines.org.uk/emc").hostname;
