...
const SEARCH_URL  = "https://publichealth.jmir.org/search?q={q}";
const SEL_RESULT  = ".search-result-row, .article-list-item";
const SEL_TITLE   = "h2 a, h3 a";
const SEL_LINK    = "h2 a, h3 a";
const SEL_SNIPPET = ".abstract, .summary";
const ORIGIN      = "https://publichealth.jmir.org";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search JMIR Public Health & Surveillance for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://publichealth.jmir.org").toString(); } catch { return; }
      ...
      return { source: "jmir-publichealth", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a JMIR Public Health & Surveillance page and return cleaned text content.",
