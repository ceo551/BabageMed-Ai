...
export function registerTools(server: McpServer) {
  server.tool({
    ...
    handler: async ({ query, limit = 10 }) => {
      const url = "https://www.webmd.com/search/search_results/default.aspx?query={q}".replace("{q}", encodeURIComponent(query));
      const r = await scraper.fetchHtml(url, { browser: true });
      const $ = r.$;
      const results: any[] = [];
      $(".search-results-doc-container").each((_, el) => {
        if (results.length >= limit) return;
        const a = $(el).find("a").first();
        const href = a.attr("href"); if (!href) return;
        try {
          const abs = new URL(href, "https://www.webmd.com").toString();
          if (!abs.startsWith("https://www.webmd.com")) return;
          results.push({ title: a.text().trim(), url: abs, snippet: $(el).find("p").first().text().trim() });
        } catch {}
      });
      return { source: "webmd", query, count: results.length, results };
    },
  });
  server.tool({
    name: "fetch",
    description: "Fetch a webmd URL and return cleaned text content.",
    ...
    handler: async ({ url }) => {
      ...
      const sel = ".article-page, .article, main";
