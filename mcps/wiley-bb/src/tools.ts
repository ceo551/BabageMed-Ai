...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Wiley Brain and Behavior for a query and return links + snippets.",
    input: z.object({ query: z.string().min(1), limit: z.number().int().min(1).max(50).optional() }),
    handler: async ({ query, limit = 10 }) => {
      const url = "https://onlinelibrary.wiley.com/journal/21623279" + "/?s=" + encodeURIComponent(query);
      ...
      $("a").each((_, a) => {
        ...
        try {
          const abs = new URL(href, "https://onlinelibrary.wiley.com/journal/21623279").toString();
      ...
      return { source: "wiley-bb", query, count: results.length, results };
    },
  });
