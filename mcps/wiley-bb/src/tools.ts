...
const SEARCH_URL  = "https://onlinelibrary.wiley.com/journal/21623279/action/doSearch?AllField={q}";
...
const ORIGIN      = "https://onlinelibrary.wiley.com/journal/21623279";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Wiley Brain and Behavior for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://onlinelibrary.wiley.com/journal/21623279").toString(); } catch { return; }
      ...
      return { source: "wiley-bb", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a Wiley Brain and Behavior page and return cleaned text content.",
    ...
    handler: async ({ url }) => {
      ...
      try {
        const u = new URL(url);
        const expected = new URL("https://onlinelibrary.wiley.com/journal/21623279").hostname;
