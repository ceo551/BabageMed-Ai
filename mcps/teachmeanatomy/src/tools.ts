...
const SEARCH_URL  = "https://teachmeanatomy.info/?s={q}";
const SEL_RESULT  = "article, .post, .search-result, li";
const SEL_TITLE   = "h1, h2, h3, .title, a";
const SEL_LINK    = "a";
const SEL_SNIPPET = "p, .excerpt, .summary";
const ORIGIN      = "https://teachmeanatomy.info";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search TeachMeAnatomy for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://teachmeanatomy.info").toString(); } catch { return; }
      ...
      return { source: "teachmeanatomy", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a TeachMeAnatomy page and return cleaned text content.",
