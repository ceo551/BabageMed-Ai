...
const SEARCH_URL  = "https://gut.bmj.com/search/{q}";
...
const ORIGIN      = "https://gut.bmj.com";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Gut (BMJ) for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://gut.bmj.com").toString(); } catch { return; }
      ...
      return { source: "gut", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a Gut (BMJ) page and return cleaned text content.",
