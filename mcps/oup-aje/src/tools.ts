...
const SEARCH_URL  = "https://academic.oup.com/aje/search-results?page=1&q={q}";
...
const ORIGIN      = "https://academic.oup.com/aje";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search American Journal of Epidemiology for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://academic.oup.com/aje").toString(); } catch { return; }
      ...
      return { source: "oup-aje", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a American Journal of Epidemiology page and return cleaned text content.",
    ...
    handler: async ({ url }) => {
      ...
      try {
        const u = new URL(url);
        const expected = new URL("https://academic.oup.com/aje").hostname;
