...
const SEARCH_URL  = "https://jkma.org/search.php?where=aview&query={q}";
const SEL_RESULT  = ".article, li";
const SEL_TITLE   = "a.title, h3 a";
const SEL_LINK    = "a.title, h3 a";
const SEL_SNIPPET = ".abstract";
const ORIGIN      = "https://jkma.org";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Korean Medical Association Journal for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://jkma.org").toString(); } catch { return; }
      ...
      return { source: "jkma", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a Korean Medical Association Journal page and return cleaned text content.",
    ...
    handler: async ({ url }) => {
      ...
      try {
        const u = new URL(url);
        const expected = new URL("https://jkma.org").hostname;
