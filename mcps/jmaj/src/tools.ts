...
const SEARCH_URL  = "https://www.jmaj.jp/search/?keyword={q}";
const SEL_RESULT  = ".search-result, .article-item";
const SEL_TITLE   = "h3 a";
const SEL_LINK    = "h3 a";
const SEL_SNIPPET = ".excerpt";
const ORIGIN      = "https://www.jmaj.jp";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search JMA Journal (Japan) for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://www.jmaj.jp").toString(); } catch { return; }
      ...
      return { source: "jmaj", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a JMA Journal (Japan) page and return cleaned text content.",
