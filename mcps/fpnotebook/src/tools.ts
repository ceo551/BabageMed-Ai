...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search fpnotebook.",
    input: z.object({ query: z.string().min(1), limit: z.number().int().min(1).max(50).optional() }),
    handler: async ({ query, limit = 10 }) => {
      const url = "https://fpnotebook.com/Search.aspx?q={q}".replace("{q}", encodeURIComponent(query));
      ...
      $("a").each((_, a) => {
        ...
        try {
          const abs = new URL(href, "https://fpnotebook.com").toString();
          if (!abs.startsWith("https://fpnotebook.com")) return;
          if (results.some(r => r.url === abs)) return;
          results.push({ title: text.slice(0, 200), url: abs, snippet: "" });
        } catch {}
      });
      return { source: "fpnotebook", query, count: results.length, results };
    },
  });
  server.tool({
    name: "fetch",
    description: "Fetch a fpnotebook URL and return cleaned text content.",
