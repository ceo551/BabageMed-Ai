...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search coreem.",
    input: z.object({ query: z.string().min(1), limit: z.number().int().min(1).max(50).optional() }),
    handler: async ({ query, limit = 10 }) => {
      const url = "https://coreem.net/?s={q}".replace("{q}", encodeURIComponent(query));
      ...
      $("a").each((_, a) => {
        ...
        try {
          const abs = new URL(href, "https://coreem.net").toString();
          if (!abs.startsWith("https://coreem.net")) return;
          if (results.some(r => r.url === abs)) return;
          results.push({ title: text.slice(0, 200), url: abs, snippet: "" });
        } catch {}
      });
      return { source: "coreem", query, count: results.length, results };
    },
  });
  server.tool({
    name: "fetch",
    description: "Fetch a coreem URL and return cleaned text content.",
