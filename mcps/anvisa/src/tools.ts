...
const SEARCH_URL  = "https://www.gov.br/anvisa/pt-br/@@busca?SearchableText={q}";
...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search ANVISA Brazil for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://www.gov.br/anvisa").toString(); } catch { return; }
      ...
      return { source: "anvisa", query, searchUrl: url, count: results.length, results };
    },
  });
