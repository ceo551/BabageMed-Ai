...
const SEARCH_URL  = "https://www.omim.org/search/?index=entry&search={q}";
const SEL_RESULT  = ".result-item";
const SEL_TITLE   = "a";
const SEL_LINK    = "a";
const SEL_SNIPPET = ".result-summary";
const ORIGIN      = "https://www.omim.org";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search OMIM for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://www.omim.org").toString(); } catch { return; }
      ...
      return { source: "omim", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a OMIM page and return cleaned text content.",
