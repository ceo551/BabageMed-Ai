...
const SEARCH_URL  = "https://www.openanesthesia.org/?s={q}";
const SEL_RESULT  = "article, .post, .search-result";
const SEL_TITLE   = "h2 a, h3 a, .entry-title a";
const SEL_LINK    = "h2 a, h3 a, .entry-title a";
const SEL_SNIPPET = ".entry-summary, .excerpt, p";
const ORIGIN      = "https://www.openanesthesia.org";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search OpenAnesthesia for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://www.openanesthesia.org").toString(); } catch { return; }
      ...
      return { source: "openanesthesia", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a OpenAnesthesia page and return cleaned text content.",
    ...
    handler: async ({ url }) => {
      ...
      try {
        const u = new URL(url);
        const expected = new URL("https://www.openanesthesia.org").hostname;
