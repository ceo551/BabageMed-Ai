...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search rsna.",
    input: z.object({ query: z.string().min(1), limit: z.number().int().min(1).max(50).optional() }),
    handler: async ({ query, limit = 10 }) => {
      const url = "https://www.rsna.org/search-results?q={q}".replace("{q}", encodeURIComponent(query));
      ...
      $("a").each((_, a) => {
        ...
        try {
          const abs = new URL(href, "https://www.rsna.org").toString();
          if (!abs.startsWith("https://www.rsna.org")) return;
          if (results.some(r => r.url === abs)) return;
          results.push({ title: text.slice(0, 200), url: abs, snippet: "" });
        } catch {}
      });
      return { source: "rsna", query, count: results.length, results };
    },
  });
  server.tool({
    name: "fetch",
    description: "Fetch a rsna URL and return cleaned text content.",
