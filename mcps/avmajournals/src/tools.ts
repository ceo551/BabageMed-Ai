...
const SEARCH_URL  = "https://avmajournals.avma.org/search?q={q}";
const SEL_RESULT  = ".al-article-items, .search-result-item, .article-list-item";
const SEL_TITLE   = ".sri-title a, h3 a";
const SEL_LINK    = ".sri-title a, h3 a";
const SEL_SNIPPET = ".sri-snippet";
const ORIGIN      = "https://avmajournals.avma.org";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search AVMA Journals for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://avmajournals.avma.org").toString(); } catch { return; }
      ...
      return { source: "avmajournals", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a AVMA Journals page and return cleaned text content.",
