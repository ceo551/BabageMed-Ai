...
// Perplexity Finance — connector observed in the Perplexity Computer Connectors
...
// authenticate.
const HOMEPAGE = "https://www.perplexity.ai/finance";
const NOT_CONFIGURED = {
  status: "not-configured",
  message: "Perplexity Finance connector is registered but not yet authenticated. Add credentials in Settings → Connectors.",
  homepage: HOMEPAGE,
};
...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Perplexity Finance. Returns 'not-configured' until OAuth/API credentials are supplied.",
  ...
  server.tool({
    name: "fetch",
    description: "Fetch a record from Perplexity Finance by id or URL. Stub until configured.",
