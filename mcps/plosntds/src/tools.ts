...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search PLOS Neglected Tropical Diseases for a query and return links + snippets.",
    input: z.object({ query: z.string().min(1), limit: z.number().int().min(1).max(50).optional() }),
    handler: async ({ query, limit = 10 }) => {
      const url = "https://journals.plos.org/plosntds" + "/?s=" + encodeURIComponent(query);
      ...
      $("a").each((_, a) => {
        ...
        try {
          const abs = new URL(href, "https://journals.plos.org/plosntds").toString();
      ...
      return { source: "plosntds", query, count: results.length, results };
    },
  });
