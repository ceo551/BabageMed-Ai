...
const SEARCH_URL  = "https://journals.plos.org/plosntds/search?q={q}";
...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search PLOS Neglected Tropical Diseases for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://journals.plos.org/plosntds").toString(); } catch { return; }
      ...
      return { source: "plosntds", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a PLOS Neglected Tropical Diseases page and return cleaned text content.",
