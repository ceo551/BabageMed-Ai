...
const SEARCH_URL  = "https://mhealth.jmir.org/search?q={q}";
...
const ORIGIN      = "https://mhealth.jmir.org";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search JMIR mHealth & uHealth for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://mhealth.jmir.org").toString(); } catch { return; }
      ...
      return { source: "jmir-mhealth", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a JMIR mHealth & uHealth page and return cleaned text content.",
