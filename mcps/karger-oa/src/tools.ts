...
const SEARCH_URL  = "https://karger.com/search-results?fl_SiteID=1&q={q}";
const SEL_RESULT  = ".search-result-item, .al-article-items";
const SEL_TITLE   = "h3 a, .sri-title a";
const SEL_LINK    = "h3 a, .sri-title a";
const SEL_SNIPPET = ".sri-snippet";
const ORIGIN      = "https://karger.com/services/open-access";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Karger Open Access for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://karger.com/services/open-access").toString(); } catch { return; }
      ...
      return { source: "karger-oa", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a Karger Open Access page and return cleaned text content.",
    ...
    handler: async ({ url }) => {
      ...
      try {
        const u = new URL(url);
        const expected = new URL("https://karger.com/services/open-access").hostname;
