...
const SEARCH_URL  = "https://jamanetwork.com/searchresults?q={q}&f_JournalDisplayName=JAMA+Network+Open";
const SEL_RESULT  = "article, .post, .search-result, li";
const SEL_TITLE   = "h1, h2, h3, .title, a";
const SEL_LINK    = "a";
const SEL_SNIPPET = "p, .excerpt, .summary";
const ORIGIN      = "https://jamanetwork.com/journals/jamanetworkopen";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search JAMA Network Open for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://jamanetwork.com/journals/jamanetworkopen").toString(); } catch { return; }
      ...
      return { source: "jamanetworkopen", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a JAMA Network Open page and return cleaned text content.",
    ...
    handler: async ({ url }) => {
      ...
      try {
        const u = new URL(url);
        const expected = new URL("https://jamanetwork.com/journals/jamanetworkopen").hostname;
