...
const SEARCH_URL  = "https://bmccancer.biomedcentral.com/search?query={q}";
...
const ORIGIN      = "https://bmccancer.biomedcentral.com";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search BMC Cancer for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://bmccancer.biomedcentral.com").toString(); } catch { return; }
      ...
      return { source: "bmccancer", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a BMC Cancer page and return cleaned text content.",
