...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search MDPI Diagnostics for a query and return links + snippets.",
    input: z.object({ query: z.string().min(1), limit: z.number().int().min(1).max(50).optional() }),
    handler: async ({ query, limit = 10 }) => {
      const url = "https://www.mdpi.com/journal/diagnostics" + "/?s=" + encodeURIComponent(query);
      ...
      $("a").each((_, a) => {
        ...
        try {
          const abs = new URL(href, "https://www.mdpi.com/journal/diagnostics").toString();
      ...
      return { source: "mdpi-diagnostics", query, count: results.length, results };
    },
  });
