...
const SEARCH_URL  = "https://casereports.bmj.com/search/{q}";
...
const ORIGIN      = "https://casereports.bmj.com";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search BMJ Case Reports for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://casereports.bmj.com").toString(); } catch { return; }
      ...
      return { source: "bmjcases", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a BMJ Case Reports page and return cleaned text content.",
