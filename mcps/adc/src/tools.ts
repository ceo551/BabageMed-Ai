...
const SEARCH_URL  = "https://adc.bmj.com/search/{q}";
...
const ORIGIN      = "https://adc.bmj.com";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Archives of Disease in Childhood (BMJ) for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://adc.bmj.com").toString(); } catch { return; }
      ...
      return { source: "adc", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a Archives of Disease in Childhood (BMJ) page and return cleaned text content.",
