...
const SEARCH_URL  = "https://bnfc.nice.org.uk/search?q={q}";
const SEL_RESULT  = ".search-results-list li";
const SEL_TITLE   = "h3 a";
const SEL_LINK    = "h3 a";
const SEL_SNIPPET = "p";
const ORIGIN      = "https://bnfc.nice.org.uk";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search British National Formulary for Children for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://bnfc.nice.org.uk").toString(); } catch { return; }
      ...
      return { source: "bnfc", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a British National Formulary for Children page and return cleaned text content.",
