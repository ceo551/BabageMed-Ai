...
const SEARCH_URL  = "https://www.asco.org/search?keys={q}";
const SEL_RESULT  = ".search-result, article";
const SEL_TITLE   = "h3 a";
const SEL_LINK    = "h3 a";
const SEL_SNIPPET = "p";
const ORIGIN      = "https://www.asco.org";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search American Society of Clinical Oncology for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://www.asco.org").toString(); } catch { return; }
      ...
      return { source: "asco", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a American Society of Clinical Oncology page and return cleaned text content.",
    ...
    handler: async ({ url }) => {
      ...
      try {
        const u = new URL(url);
        const expected = new URL("https://www.asco.org").hostname;
