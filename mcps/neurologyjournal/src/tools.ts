...
const SEARCH_URL  = "https://www.neurology.org/action/doSearch?AllField={q}";
const SEL_RESULT  = ".articleListItem";
const SEL_TITLE   = ".articleTitle a";
const SEL_LINK    = ".articleTitle a";
const SEL_SNIPPET = ".articleSnippet";
const ORIGIN      = "https://www.neurology.org";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Neurology Journal (AAN) for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://www.neurology.org").toString(); } catch { return; }
      ...
      return { source: "neurologyjournal", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a Neurology Journal (AAN) page and return cleaned text content.",
