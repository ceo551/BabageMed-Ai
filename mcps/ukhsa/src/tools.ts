...
const SEARCH_URL  = "https://www.gov.uk/search/all?keywords={q}&organisations%5B%5D=uk-health-security-agency";
...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search UK Health Security Agency for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://www.gov.uk/government/organisations/uk-health-security-agency").toString(); } catch { return; }
      ...
      return { source: "ukhsa", query, searchUrl: url, count: results.length, results };
    },
  });
