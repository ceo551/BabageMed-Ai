...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search UK Health Security Agency for a query and return links + snippets.",
    input: z.object({ query: z.string().min(1), limit: z.number().int().min(1).max(50).optional() }),
    handler: async ({ query, limit = 10 }) => {
      const url = "https://www.gov.uk/government/organisations/uk-health-security-agency" + "/?s=" + encodeURIComponent(query);
      ...
      $("a").each((_, a) => {
        ...
        try {
          const abs = new URL(href, "https://www.gov.uk/government/organisations/uk-health-security-agency").toString();
          if (!abs.startsWith("https://www.gov.uk")) return;
          if (results.some(x => x.url === abs)) return;
          results.push({ title: text.slice(0, 200), url: abs, snippet: "" });
        } catch {}
      });
      return { source: "ukhsa", query, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a UK Health Security Agency page and return cleaned text content.",
