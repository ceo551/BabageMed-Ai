...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search MDPI Journal of Clinical Medicine for a query and return links + snippets.",
    input: z.object({ query: z.string().min(1), limit: z.number().int().min(1).max(50).optional() }),
    handler: async ({ query, limit = 10 }) => {
      const url = "https://www.mdpi.com/journal/jcm" + "/?s=" + encodeURIComponent(query);
      ...
      $("a").each((_, a) => {
        ...
        try {
          const abs = new URL(href, "https://www.mdpi.com/journal/jcm").toString();
          if (!abs.startsWith("https://www.mdpi.com")) return;
          if (results.some(x => x.url === abs)) return;
          results.push({ title: text.slice(0, 200), url: abs, snippet: "" });
        } catch {}
      });
      return { source: "mdpi-jcm", query, count: results.length, results };
    },
  });
