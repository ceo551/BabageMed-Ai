...
const SEARCH_URL  = "https://www.mdpi.com/search?q={q}&journal=jcm";
const SEL_RESULT  = ".article-content";
const SEL_TITLE   = ".title-link";
const SEL_LINK    = ".title-link";
const SEL_SNIPPET = ".abstract-cropped";
const ORIGIN      = "https://www.mdpi.com";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search MDPI Journal of Clinical Medicine for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://www.mdpi.com/journal/jcm").toString(); } catch { return; }
      ...
      return { source: "mdpi-jcm", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a MDPI Journal of Clinical Medicine page and return cleaned text content.",
