...
const SEARCH_URL  = "https://search.scielo.org/?q={q}&lang=en";
const SEL_RESULT  = ".item, li";
const SEL_TITLE   = ".titulo a, h2 a";
const SEL_LINK    = ".titulo a, h2 a";
const SEL_SNIPPET = ".paragraph";
const ORIGIN      = "https://www.scielo.br";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search SciELO Brazil for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://www.scielo.br").toString(); } catch { return; }
      ...
      return { source: "scielo", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a SciELO Brazil page and return cleaned text content.",
