...
const SEARCH_URL  = "https://www.thelancet.com/action/doSearch?AllField={q}";
...
const ORIGIN      = "https://www.thelancet.com";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search The Lancet for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://www.thelancet.com").toString(); } catch { return; }
      ...
      return { source: "lancet", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a The Lancet page and return cleaned text content.",
