...
const SEARCH_URL  = "https://www.ahajournals.org/action/doSearch?AllField={q}";
...
const ORIGIN      = "https://www.ahajournals.org";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search American Heart Association Journals for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://www.ahajournals.org").toString(); } catch { return; }
      ...
      return { source: "ahajournals", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a American Heart Association Journals page and return cleaned text content.",
    ...
    handler: async ({ url }) => {
      ...
      try {
        const u = new URL(url);
        const expected = new URL("https://www.ahajournals.org").hostname;
