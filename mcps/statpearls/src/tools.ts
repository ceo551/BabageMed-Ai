...
const SEARCH_URL  = "https://www.statpearls.com/articlelibrary/searchresult?KW={q}";
const SEL_RESULT  = ".article-card, li";
const SEL_TITLE   = "a";
const SEL_LINK    = "a";
const SEL_SNIPPET = ".excerpt";
const ORIGIN      = "https://www.statpearls.com";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search StatPearls for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://www.statpearls.com").toString(); } catch { return; }
      ...
      return { source: "statpearls", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a StatPearls page and return cleaned text content.",
    ...
    handler: async ({ url }) => {
      ...
      try {
        const u = new URL(url);
        const expected = new URL("https://www.statpearls.com").hostname;
