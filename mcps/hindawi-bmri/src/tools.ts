...
const SEARCH_URL  = "https://www.hindawi.com/search/all/?q={q}";
const SEL_RESULT  = ".search-result, .search-result-list-item";
const SEL_TITLE   = "h2 a, h3 a";
const SEL_LINK    = "h2 a, h3 a";
const SEL_SNIPPET = ".abstract";
const ORIGIN      = "https://www.hindawi.com";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Hindawi BioMed Research International for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://www.hindawi.com/journals/bmri/").toString(); } catch { return; }
      ...
      return { source: "hindawi-bmri", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a Hindawi BioMed Research International page and return cleaned text content.",
