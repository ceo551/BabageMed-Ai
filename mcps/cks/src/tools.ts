...
const SEARCH_URL  = "https://cks.nice.org.uk/search?q={q}";
const SEL_RESULT  = ".search-results-list li, article";
const SEL_TITLE   = "h3 a";
const SEL_LINK    = "h3 a";
const SEL_SNIPPET = "p";
const ORIGIN      = "https://cks.nice.org.uk";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search NICE Clinical Knowledge Summaries (CKS) for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://cks.nice.org.uk").toString(); } catch { return; }
      ...
      return { source: "cks", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a NICE Clinical Knowledge Summaries (CKS) page and return cleaned text content.",
