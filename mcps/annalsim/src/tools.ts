...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Annals of Internal Medicine for a query and return links + snippets.",
    input: z.object({ query: z.string().min(1), limit: z.number().int().min(1).max(50).optional() }),
    handler: async ({ query, limit = 10 }) => {
      const url = "https://www.acpjournals.org/journal/aim" + "/?s=" + encodeURIComponent(query);
      ...
      $("a").each((_, a) => {
        ...
        try {
          const abs = new URL(href, "https://www.acpjournals.org/journal/aim").toString();
          if (!abs.startsWith("https://www.acpjournals.org")) return;
          if (results.some(x => x.url === abs)) return;
          results.push({ title: text.slice(0, 200), url: abs, snippet: "" });
        } catch {}
      });
      return { source: "annalsim", query, count: results.length, results };
    },
  });
