...
const SEARCH_URL  = "https://www.acpjournals.org/journal/aim/action/doSearch?AllField={q}";
const SEL_RESULT  = ".articleListItem, .search-result-item";
const SEL_TITLE   = ".articleTitle a, h3 a";
const SEL_LINK    = ".articleTitle a, h3 a";
const SEL_SNIPPET = ".articleSnippet";
const ORIGIN      = "https://www.acpjournals.org/journal/aim";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Annals of Internal Medicine for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://www.acpjournals.org/journal/aim").toString(); } catch { return; }
      ...
      return { source: "annalsim", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a Annals of Internal Medicine page and return cleaned text content.",
    ...
    handler: async ({ url }) => {
      ...
      try {
        const u = new URL(url);
        const expected = new URL("https://www.acpjournals.org/journal/aim").hostname;
