...
const SEARCH_URL  = "https://bjsm.bmj.com/search/{q}";
const SEL_RESULT  = ".highwire-search-result, .search-result";
const SEL_TITLE   = "h2 a, .highwire-cite-title a";
const SEL_LINK    = "h2 a, .highwire-cite-title a";
const SEL_SNIPPET = ".highwire-search-result-snippet";
const ORIGIN      = "https://bjsm.bmj.com";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search British Journal of Sports Medicine (BMJ) for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://bjsm.bmj.com").toString(); } catch { return; }
      ...
      return { source: "bjsm", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a British Journal of Sports Medicine (BMJ) page and return cleaned text content.",
