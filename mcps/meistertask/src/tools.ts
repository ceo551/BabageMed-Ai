...
// MeisterTask — connector observed in the Perplexity Computer Connectors
...
// authenticate.
const HOMEPAGE = "https://www.meistertask.com";
const NOT_CONFIGURED = {
  status: "not-configured",
  message: "MeisterTask connector is registered but not yet authenticated. Add credentials in Settings → Connectors.",
  homepage: HOMEPAGE,
};
...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search MeisterTask. Returns 'not-configured' until OAuth/API credentials are supplied.",
  ...
  server.tool({
    name: "fetch",
    description: "Fetch a record from MeisterTask by id or URL. Stub until configured.",
