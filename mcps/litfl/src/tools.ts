...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search litfl.",
    input: z.object({ query: z.string().min(1), limit: z.number().int().min(1).max(50).optional() }),
    handler: async ({ query, limit = 10 }) => {
      const url = "https://litfl.com/?s={q}".replace("{q}", encodeURIComponent(query));
      ...
      $(".search-result, article").each((_, el) => {
        ...
        try {
          const abs = new URL(href, "https://litfl.com").toString();
          if (!abs.startsWith("https://litfl.com")) return;
          results.push({ title: a.text().trim(), url: abs, snippet: $(el).find("p").first().text().trim() });
        } catch {}
      });
      return { source: "litfl", query, count: results.length, results };
    },
  });
  server.tool({
    name: "fetch",
    description: "Fetch a litfl URL and return cleaned text content.",
