...
const SEARCH_URL  = "https://www.eurosurveillance.org/search?q={q}";
const SEL_RESULT  = ".searchResultItem";
const SEL_TITLE   = ".articleTitle a";
const SEL_LINK    = ".articleTitle a";
const SEL_SNIPPET = ".articleSnippet";
const ORIGIN      = "https://www.eurosurveillance.org";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Eurosurveillance for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://www.eurosurveillance.org").toString(); } catch { return; }
      ...
      return { source: "eurosurveillance", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a Eurosurveillance page and return cleaned text content.",
