...
const SEARCH_URL  = "https://ramb.amb.org.br/search?q={q}";
const SEL_RESULT  = ".article-item, li";
const SEL_TITLE   = "h3 a, a.title";
const SEL_LINK    = "h3 a, a.title";
const SEL_SNIPPET = ".excerpt";
const ORIGIN      = "https://ramb.amb.org.br";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Revista da AMB for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://ramb.amb.org.br").toString(); } catch { return; }
      ...
      return { source: "ramb", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a Revista da AMB page and return cleaned text content.",
    ...
    handler: async ({ url }) => {
      ...
      try {
        const u = new URL(url);
        const expected = new URL("https://ramb.amb.org.br").hostname;
