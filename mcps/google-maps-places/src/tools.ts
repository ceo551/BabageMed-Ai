...
// Google Maps (Places API) — connector observed in the Perplexity Computer Connectors
...
// authenticate.
const HOMEPAGE = "https://developers.google.com/maps";
const NOT_CONFIGURED = {
  status: "not-configured",
  message: "Google Maps (Places API) connector is registered but not yet authenticated. Add credentials in Settings → Connectors.",
  homepage: HOMEPAGE,
};
...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Google Maps (Places API). Returns 'not-configured' until OAuth/API credentials are supplied.",
  ...
  server.tool({
    name: "fetch",
    description: "Fetch a record from Google Maps (Places API) by id or URL. Stub until configured.",
