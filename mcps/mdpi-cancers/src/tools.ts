...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search MDPI Cancers for a query and return links + snippets.",
    input: z.object({ query: z.string().min(1), limit: z.number().int().min(1).max(50).optional() }),
    handler: async ({ query, limit = 10 }) => {
      const url = "https://www.mdpi.com/journal/cancers" + "/?s=" + encodeURIComponent(query);
      ...
      $("a").each((_, a) => {
        ...
        try {
          const abs = new URL(href, "https://www.mdpi.com/journal/cancers").toString();
      ...
      return { source: "mdpi-cancers", query, count: results.length, results };
    },
  });
