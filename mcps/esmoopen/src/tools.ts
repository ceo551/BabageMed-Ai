...
const SEARCH_URL  = "https://www.esmoopen.com/action/doSearch?AllField={q}";
...
const ORIGIN      = "https://www.esmoopen.com";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search ESMO Open for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://www.esmoopen.com").toString(); } catch { return; }
      ...
      return { source: "esmoopen", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a ESMO Open page and return cleaned text content.",
