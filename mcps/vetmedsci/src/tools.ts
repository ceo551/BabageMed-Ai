...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Veterinary Medicine and Science for a query and return links + snippets.",
    input: z.object({ query: z.string().min(1), limit: z.number().int().min(1).max(50).optional() }),
    handler: async ({ query, limit = 10 }) => {
      const url = "https://onlinelibrary.wiley.com/journal/20531095" + "/?s=" + encodeURIComponent(query);
      ...
      $("a").each((_, a) => {
        ...
        try {
          const abs = new URL(href, "https://onlinelibrary.wiley.com/journal/20531095").toString();
          if (!abs.startsWith("https://onlinelibrary.wiley.com")) return;
          if (results.some(x => x.url === abs)) return;
          results.push({ title: text.slice(0, 200), url: abs, snippet: "" });
        } catch {}
      });
      return { source: "vetmedsci", query, count: results.length, results };
    },
  });
