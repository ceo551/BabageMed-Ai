...
const SEARCH_URL  = "https://onlinelibrary.wiley.com/journal/20531095/action/doSearch?AllField={q}";
const SEL_RESULT  = ".search__item, .articleListItem";
const SEL_TITLE   = ".publication_title a, h3 a";
const SEL_LINK    = ".publication_title a, h3 a";
const SEL_SNIPPET = ".search-result-snippet";
const ORIGIN      = "https://onlinelibrary.wiley.com/journal/20531095";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Veterinary Medicine and Science for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://onlinelibrary.wiley.com/journal/20531095").toString(); } catch { return; }
      ...
      return { source: "vetmedsci", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a Veterinary Medicine and Science page and return cleaned text content.",
    ...
    handler: async ({ url }) => {
      ...
      try {
        const u = new URL(url);
        const expected = new URL("https://onlinelibrary.wiley.com/journal/20531095").hostname;
