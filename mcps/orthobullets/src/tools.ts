...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Orthobullets for a query and return links + snippets.",
    input: z.object({ query: z.string().min(1), limit: z.number().int().min(1).max(50).optional() }),
    handler: async ({ query, limit = 10 }) => {
      const url = "https://www.orthobullets.com" + "/?s=" + encodeURIComponent(query);
      ...
      $("a").each((_, a) => {
        ...
        try {
          const abs = new URL(href, "https://www.orthobullets.com").toString();
          if (!abs.startsWith("https://www.orthobullets.com")) return;
          if (results.some(x => x.url === abs)) return;
          results.push({ title: text.slice(0, 200), url: abs, snippet: "" });
        } catch {}
      });
      return { source: "orthobullets", query, count: results.length, results };
    },
  });
