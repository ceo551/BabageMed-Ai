...
const SEARCH_URL  = "https://www.orthobullets.com/search?q={q}";
const SEL_RESULT  = ".search-result, li";
const SEL_TITLE   = "a";
const SEL_LINK    = "a";
const SEL_SNIPPET = ".excerpt";
const ORIGIN      = "https://www.orthobullets.com";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Orthobullets for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://www.orthobullets.com").toString(); } catch { return; }
      ...
      return { source: "orthobullets", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a Orthobullets page and return cleaned text content.",
    ...
    handler: async ({ url }) => {
      ...
      try {
        const u = new URL(url);
        const expected = new URL("https://www.orthobullets.com").hostname;
