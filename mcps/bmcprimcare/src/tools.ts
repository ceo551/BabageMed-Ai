...
const SEARCH_URL  = "https://bmcprimcare.biomedcentral.com/search?query={q}";
...
const ORIGIN      = "https://bmcprimcare.biomedcentral.com";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search BMC Primary Care for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://bmcprimcare.biomedcentral.com").toString(); } catch { return; }
      ...
      return { source: "bmcprimcare", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a BMC Primary Care page and return cleaned text content.",
