...
const SEARCH_URL  = "https://www.cureus.com/articles?searchquery={q}";
const SEL_RESULT  = ".article-card, .search-result";
const SEL_TITLE   = "h2 a, .article-card__title a";
const SEL_LINK    = "h2 a, .article-card__title a";
const SEL_SNIPPET = ".article-card__abstract";
const ORIGIN      = "https://www.cureus.com";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Cureus for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://www.cureus.com").toString(); } catch { return; }
      ...
      return { source: "cureus", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a Cureus page and return cleaned text content.",
    ...
    handler: async ({ url }) => {
      ...
      try {
        const u = new URL(url);
        const expected = new URL("https://www.cureus.com").hostname;
