...
const SEARCH_URL  = "https://www.rcpch.ac.uk/search?keyword={q}";
const SEL_RESULT  = "article, .post, .search-result, li";
const SEL_TITLE   = "h1, h2, h3, .title, a";
const SEL_LINK    = "a";
const SEL_SNIPPET = "p, .excerpt, .summary";
const ORIGIN      = "https://www.rcpch.ac.uk";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Royal College of Paediatrics & Child Health for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://www.rcpch.ac.uk").toString(); } catch { return; }
      ...
      return { source: "rcpch", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a Royal College of Paediatrics & Child Health page and return cleaned text content.",
    ...
    handler: async ({ url }) => {
      ...
      try {
        const u = new URL(url);
        const expected = new URL("https://www.rcpch.ac.uk").hostname;
