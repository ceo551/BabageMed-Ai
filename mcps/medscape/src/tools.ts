...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search medscape.",
    input: z.object({ query: z.string().min(1), limit: z.number().int().min(1).max(50).optional() }),
    handler: async ({ query, limit = 10 }) => {
      const url = "https://search.medscape.com/search/?q={q}".replace("{q}", encodeURIComponent(query));
      ...
      $(".searchResult").each((_, el) => {
        ...
        try {
          const abs = new URL(href, "https://www.medscape.com").toString();
          if (!abs.startsWith("https://www.medscape.com")) return;
          results.push({ title: a.text().trim(), url: abs, snippet: $(el).find("p").first().text().trim() });
        } catch {}
      });
      return { source: "medscape", query, count: results.length, results };
    },
  });
  server.tool({
    name: "fetch",
    description: "Fetch a medscape URL and return cleaned text content.",
    ...
    handler: async ({ url }) => {
      ...
      const sel = ".article-content, main, article";
