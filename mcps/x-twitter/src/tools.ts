...
// X (Twitter) — connector observed in the Perplexity Computer Connectors
...
// authenticate.
const HOMEPAGE = "https://x.com";
const NOT_CONFIGURED = {
  status: "not-configured",
  message: "X (Twitter) connector is registered but not yet authenticated. Add credentials in Settings → Connectors.",
  homepage: HOMEPAGE,
};
...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search X (Twitter). Returns 'not-configured' until OAuth/API credentials are supplied.",
  ...
  server.tool({
    name: "fetch",
    description: "Fetch a record from X (Twitter) by id or URL. Stub until configured.",
