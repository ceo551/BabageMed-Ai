...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Google Workspace Admin for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      return { source: "google-workspace-admin", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a Google Workspace Admin page and return cleaned text content.",
