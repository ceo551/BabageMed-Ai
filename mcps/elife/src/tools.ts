...
const SEARCH_URL  = "https://elifesciences.org/search?for={q}";
const SEL_RESULT  = ".teaser, .search-result";
const SEL_TITLE   = ".teaser__header_text a, h2 a";
const SEL_LINK    = ".teaser__header_text a, h2 a";
const SEL_SNIPPET = ".teaser__body";
const ORIGIN      = "https://elifesciences.org";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search eLife for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://elifesciences.org").toString(); } catch { return; }
      ...
      return { source: "elife", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a eLife page and return cleaned text content.",
