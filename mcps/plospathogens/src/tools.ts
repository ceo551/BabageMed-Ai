...
const SEARCH_URL  = "https://journals.plos.org/plospathogens/search?q={q}";
...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search PLOS Pathogens for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://journals.plos.org/plospathogens").toString(); } catch { return; }
      ...
      return { source: "plospathogens", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a PLOS Pathogens page and return cleaned text content.",
