...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Mux for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      return { source: "mux", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a Mux page and return cleaned text content.",
