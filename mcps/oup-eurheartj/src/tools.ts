...
const SEARCH_URL  = "https://academic.oup.com/eurheartj/search-results?page=1&q={q}";
const SEL_RESULT  = ".al-article-items, .search-result-item";
const SEL_TITLE   = ".sri-title a, .item-title a";
const SEL_LINK    = ".sri-title a, .item-title a";
const SEL_SNIPPET = ".sri-snippet, .excerpt";
const ORIGIN      = "https://academic.oup.com/eurheartj";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search European Heart Journal (Oxford) for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://academic.oup.com/eurheartj").toString(); } catch { return; }
      ...
      return { source: "oup-eurheartj", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a European Heart Journal (Oxford) page and return cleaned text content.",
    ...
    handler: async ({ url }) => {
      ...
      try {
        const u = new URL(url);
        const expected = new URL("https://academic.oup.com/eurheartj").hostname;
