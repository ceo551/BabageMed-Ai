...
const SEARCH_URL  = "https://mental.jmir.org/search?q={q}";
...
const ORIGIN      = "https://mental.jmir.org";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search JMIR Mental Health for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://mental.jmir.org").toString(); } catch { return; }
      ...
      return { source: "jmir-mental", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a JMIR Mental Health page and return cleaned text content.",
