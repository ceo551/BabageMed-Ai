...
const SEARCH_URL  = "https://jamanetwork.com/searchresults?q={q}";
const SEL_RESULT  = ".search-result, .article-item";
const SEL_TITLE   = "h3 a, .item-title a";
const SEL_LINK    = "h3 a, .item-title a";
const SEL_SNIPPET = ".snippet";
const ORIGIN      = "https://jamanetwork.com";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search JAMA Network for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://jamanetwork.com").toString(); } catch { return; }
      ...
      return { source: "jamanetwork", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a JAMA Network page and return cleaned text content.",
    ...
    handler: async ({ url }) => {
      ...
      try {
        const u = new URL(url);
        const expected = new URL("https://jamanetwork.com").hostname;
