...
const SEARCH_URL  = "https://bmcmedicine.biomedcentral.com/search?query={q}";
...
const ORIGIN      = "https://bmcmedicine.biomedcentral.com";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search BMC Medicine for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://bmcmedicine.biomedcentral.com").toString(); } catch { return; }
      ...
      return { source: "bmcmed", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a BMC Medicine page and return cleaned text content.",
