...
const scraper = new Scraper({
  base: "https://journals.plos.org/plosmedicine",
...
const SEARCH_URL  = "https://journals.plos.org/plosmedicine/search?q={q}";
const SEL_RESULT  = ".search-result, li.search-results-item";
const SEL_TITLE   = ".search-result-title a, h2 a";
const SEL_LINK    = ".search-result-title a, h2 a";
const SEL_SNIPPET = ".search-result-abstract, .abstract";
const ORIGIN      = "https://journals.plos.org";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search PLOS Medicine for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://journals.plos.org/plosmedicine").toString(); } catch { return; }
      ...
      return { source: "plosmedicine", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a PLOS Medicine page and return cleaned text content.",
