...
const SEARCH_URL  = "http://www.samj.org.za/index.php/samj/search/search?query={q}";
const SEL_RESULT  = ".article-summary, li";
const SEL_TITLE   = "a.title";
const SEL_LINK    = "a.title";
const SEL_SNIPPET = ".abstract";
const ORIGIN      = "https://samj.org.za";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search South African Medical Journal for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://samj.org.za").toString(); } catch { return; }
      ...
      return { source: "samj", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a South African Medical Journal page and return cleaned text content.",
