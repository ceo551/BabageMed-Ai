...
const SEARCH_URL  = "https://openprescribing.net/api/1.0/bnf_code/?q={q}";
const SEL_RESULT  = "article, .post, .search-result, li";
const SEL_TITLE   = "h1, h2, h3, .title, a";
const SEL_LINK    = "a";
const SEL_SNIPPET = "p, .excerpt, .summary";
const ORIGIN      = "https://openprescribing.net";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search OpenPrescribing.net for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://openprescribing.net").toString(); } catch { return; }
      ...
      return { source: "openprescribing", query, searchUrl: url, count: results.length, results };
    },
  });
  ...
  server.tool({
    ...
    handler: async ({ url }) => {
      ...
      try {
        const u = new URL(url);
        const expected = new URL("https://openprescribing.net").hostname;
