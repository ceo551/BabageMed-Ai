...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search Vet Times for a query and return links + snippets.",
    input: z.object({ query: z.string().min(1), limit: z.number().int().min(1).max(50).optional() }),
    handler: async ({ query, limit = 10 }) => {
      const url = "https://www.vettimes.co.uk" + "/?s=" + encodeURIComponent(query);
      ...
      $("a").each((_, a) => {
        ...
        try {
          const abs = new URL(href, "https://www.vettimes.co.uk").toString();
          if (!abs.startsWith("https://www.vettimes.co.uk")) return;
          if (results.some(x => x.url === abs)) return;
          results.push({ title: text.slice(0, 200), url: abs, snippet: "" });
        } catch {}
      });
      return { source: "vettimes", query, count: results.length, results };
    },
  });
