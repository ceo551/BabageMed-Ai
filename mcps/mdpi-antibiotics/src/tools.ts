...
const SEARCH_URL  = "https://www.mdpi.com/search?q={q}&journal=antibiotics";
...
export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search MDPI Antibiotics for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://www.mdpi.com/journal/antibiotics").toString(); } catch { return; }
      ...
      return { source: "mdpi-antibiotics", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a MDPI Antibiotics page and return cleaned text content.",
