...
const SEARCH_URL  = "https://bmccardiovascdisord.biomedcentral.com/search?query={q}";
const SEL_RESULT  = ".c-list-group__item, .c-article-list__item";
const SEL_TITLE   = "h3 a, .c-card__title a";
const SEL_LINK    = "h3 a, .c-card__title a";
const SEL_SNIPPET = ".c-list-group__abstract";
const ORIGIN      = "https://bmccardiovascdisord.biomedcentral.com";

export function registerTools(server: McpServer) {
  server.tool({
    name: "search",
    description: "Search BMC Cardiovascular Disorders for a query and return structured results.",
    ...
    handler: async ({ query, limit = 10 }) => {
      ...
      $(SEL_RESULT).each((_, el) => {
        ...
        if (!href) return;
        let abs: string;
        try { abs = new URL(href, "https://bmccardiovascdisord.biomedcentral.com").toString(); } catch { return; }
      ...
      return { source: "bmccardio", query, searchUrl: url, count: results.length, results };
    },
  });

  server.tool({
    name: "fetch",
    description: "Fetch a BMC Cardiovascular Disorders page and return cleaned text content.",
