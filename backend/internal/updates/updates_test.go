...
func TestHandler_NoManifestReturns204(t *testing.T) {
	...
	r := chi.NewRouter()
	svc.Register(r)
	req := httptest.NewRequest("GET", "/api/desktop/update/darwin/aarch64/0.1.0", nil)
...
func TestHandler_NewerVersionReturnsJSON(t *testing.T) {
	manifest := Manifest{
		...
		Platforms: map[string]PlatformAsset{
			"darwin-aarch64": {URL: "https://example.com/m1.app.tar.gz", Signature: "sig1"},
		},
	}
...
func TestHandler_UpToDateReturns204(t *testing.T) {
	manifest := Manifest{
		Version: "0.1.0",
		Platforms: map[string]PlatformAsset{
			"darwin-aarch64": {URL: "x", Signature: "y"},
		},
	}
	raw, _ := json.Marshal(manifest)
	svc := New(Config{Inline: string(raw)})
