...
// ── Web Push (P6.5): notify when an async agent task finishes ──────────────
self.addEventListener("push", (event) => {
  let data = {};
  try { data = event.data ? event.data.json() : {}; } catch (e) { /* opaque push */ }
  const title = data.title || "Pervagans";
  event.waitUntil(
    self.registration.showNotification(title, {
      body: data.body || "",
      icon: "/pervagans-icon.png",
      badge: "/pervagans-icon.png",
      tag: "pervagans-task",
...
self.addEventListener("notificationclick", (event) => {
  ...
  event.waitUntil(
    self.clients.matchAll({ type: "window", includeUncontrolled: true }).then((clients) => {
      // Focus an existing Pervagans tab if one is open; else open the page.
