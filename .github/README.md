...
4. **`gitops-bump`** — writes the new tag to `infra/helm/pervagans/values-images.yaml`, commits + pushes back to `main`. ArgoCD / Flux pick up the file on their next reconcile and roll the deployment. If `vars.ARGOCD_WEBHOOK_URL` is set, the job also pings it for instant sync.
