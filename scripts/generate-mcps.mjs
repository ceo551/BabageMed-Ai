...
`;
  ...
  return `import { z, McpServer, Scraper, cheerioLoad } from "@pervagans/mcp-base";
...
// Shared helpers (Google OAuth, etc.) now live in @pervagans/mcp-base
...
for (const s of MANIFEST.servers) {
  ...
  compose.push(`    image: pervagans/mcp-${s.id}:latest`);
  ...
  compose.push(`    networks: [pervagans]`);
}
compose.push("");
compose.push("networks:");
compose.push("  pervagans:");
compose.push("    name: pervagans");
...
w(join(ROOT, "infra/helm/pervagans/mcps-all.txt"), allTxt + "\n");
...
w(join(ROOT, "infra/helm/pervagans/mcps-index.json"), JSON.stringify(indexJson, null, 2) + "\n");
